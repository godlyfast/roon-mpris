...
- **Deaf-after-network-blip** (root-caused 2026-09-23): If the WebSocket connect to the Core fails before `open` fires (e.g. Wi-Fi flap), node-roon-api's `transport-websocket.js` skips `onclose` because `_isonopencalled` is false. The dead entry stays in `roon._sood_conns` keyed by core id, so `lib.js` ignores every later SOOD announce for that core (~lib.js:141-143) and the bridge never re-pairs until restarted. Observed: 3h deaf after a flap, fixed only by `systemctl --user restart roon-mpris`.
- **60s self-heal watchdog** (`core_unpaired` in index.js): after the player teardown, arms a 60s `healTimer`. When it fires and no core has paired yet, it closes+deletes every entry in `roon._sood_conns` (each in try/catch), then calls `roon.stop_discovery()` (try/catch) + `roon.start_discovery()`. Cleared in `core_paired` via `clearTimeout(healTimer)`.
- **Upstream fix** (not yet in node-roon-api): call `onclose` even when `open` never ran, so dead `_sood_conns` entries get reaped on pre-open failures. The watchdog above is the local workaround.
